...
define('CDASHMM_CRM_VERSION',	'1.5.6');
...
add_action('cdash_technical_details_hook', 'cdash_crm_render_technical_details', 30);

function cdash_crm_render_technical_details(){
  echo "<h4>CRM Version: " . CDASHMM_CRM_VERSION . "</h4>";
}

function cdash_crm_check_bd_version(){
	if ( is_admin() && current_user_can( 'activate_plugins' )){
        if(defined('CDASH_BUS_VER') && CDASH_BUS_VER < '3.1.9'){
            add_action( 'admin_notices', 'cdash_crm_update_bd_notice' );
            deactivate_plugins( plugin_basename( __FILE__ ) );
            if ( isset( $_GET['activate'] ) ) {
                unset( $_GET['activate'] );
            }    
  ...
  add_action( 'admin_init', 'cdash_crm_check_bd_version' );
  add_action( 'upgrader_process_complete', 'cdash_crm_check_bd_version');
  function cdash_crm_update_bd_notice(){
	?><div class="error"><p><?php _e('Please update Chamber Dashboard Business Directory to version 3.1.9 or later before updating CRM.', 'cdcrm' ); ?></p></div>
  <?php
  }
  
  //Block Scripts
  function cd_crm_block_scripts() {
	...
	wp_enqueue_script('cd-crm-blocks');
	wp_localize_script( 'cd-crm-blocks', 'wpAjax', array( 'wpurl' => get_bloginfo('wpurl') ) );
}
add_action( 'enqueue_block_editor_assets', 'cd_crm_block_scripts', 30 );
...
function cdash_crm_plugin_action_links( $links ) {
    $settings_url = get_admin_url() . 'admin.php?page=cd-settings&tab=crm';
    $settings_link = '<a href="' . $settings_url . '">' . __('Settings', 'cdcrm') . '</a>';
    array_unshift( $links, $settings_link );
  //}
  return $links;
}
add_action( 'plugin_action_links_' . plugin_basename( __FILE__ ), 'cdash_crm_plugin_action_links' );
...
if( defined( 'CDASH_PATH' ) ) {
	...
	function cdcrm_people_and_businesses() {
		...
		$rolesarray = array_map('trim', explode(',', $roleslist));
